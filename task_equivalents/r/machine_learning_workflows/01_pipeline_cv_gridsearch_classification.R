...
set.seed(1)
tune.out1 = tune(svm, train.x=x, train.y=y, kernel='linear', ranges=list(cost=ctry), tunecontrol=tune_ctrl)
summary(tune.out1)

polytry = c(2, 3, 4, 5)
set.seed(1)
tune.out2 = tune(svm, train.x=x, train.y=y, kernel='polynomial', ranges=list(cost=ctry, degree=polytry), tunecontrol=tune_ctrl)
summary(tune.out2)

gammatry = c(0.5, 1, 2, 3)
set.seed(1)
tune.out3 = tune(svm, train.x=x, train.y=y, kernel='radial', ranges=list(cost=ctry, gamma=gammatry), tunecontrol=tune_ctrl)
summary(tune.out3)
